...
# Imports différés pour supprimer la dépendance circulaire

def clean_text(value) -> str:
    """Normalise un texte via le module de nettoyage."""
    from scr.data.cleaning import clean_text as normalize

    return normalize(value)


def content_hash(record: dict) -> str:
    """Calcule le hash canonique sans import circulaire."""
    from scr.data.cleaning import content_hash as compute

    return compute(record)


def assemble_input(record: dict) -> str:
    """Reconstruit l'entrée sans import circulaire."""
    from scr.data.cleaning import assemble_input as assemble
...
@lru_cache(maxsize=1)
def presidio_analyzer() -> AnalyzerEngine:
    """Initialise une seule fois l'analyseur bilingue."""
    configuration = {
        ...
        "models": [
            ...
            for language, model
            in PrivacyConfig.NLP_MODELS.items()
        ]
    }
    ...
    analyzer = AnalyzerEngine(
        ...
        supported_languages=list(
            PrivacyConfig.SUPPORTED_LANGUAGES
        )
    )

    for language in PrivacyConfig.SUPPORTED_LANGUAGES:
        for entity, config in (
            PrivacyConfig.CUSTOM_RECOGNIZERS.items()
        ):
            patterns = [
                Pattern(
...
@lru_cache(maxsize=1)
def presidio_anonymizer() -> AnonymizerEngine:
    """Initialise une seule fois l'anonymiseur."""
...
class PrivacyProcessor:
    ...
    def pii_matches(
        ...
        """Retourne les PII sans exposer leur valeur."""
        matches = []

        for field, value in self.iter_text_content():
            for result in self._analyze(value):
    ...
    def explicit_names(
        ...
        """Retourne les hashes des identités explicites."""
        return {
            match["value_hash"]
            for match in self.pii_matches()
            if match["category"] == "IDENTITY_INTRODUCTION"
        }
    ...
    def apply_automatic_redactions(
        ...
        for field, value in list(self.iter_text_content()):
            results = [
                result
                for result in self._analyze(value)
            ...
            container, key = target
            container[key] = self.anonymizer.anonymize(
                text=value,
    ...
    def redact_explicit_names(
        ...
        """Compatibilité : les identités sont déjà traitées."""
        return []
    ...
    def _language(self) -> str:
        """Retourne une langue prise en charge."""
        value = str(
            self.record.get("language")
            or self.record.get("detected_language")
            or "en"
        ).lower().strip()

        return (
            value
            if value in PrivacyConfig.SUPPORTED_LANGUAGES
            else "en"
        )

    def _analyze(self, value: str) -> list:
        """Analyse un texte avec un cache local."""
        if value not in self._analysis_cache:
            results = self.analyzer.analyze(
                text=value,
                language=self.language,
                entities=list(
                    PrivacyConfig.PRESIDIO_ENTITIES
                ),
                score_threshold=(
                    PrivacyConfig.SCORE_THRESHOLD
                )
            )
            ...
            for result in results:
                if not self._keep_result(value, result):
                    continue
            ...
            self._analysis_cache[value] = sorted(
                unique.values(),
                key=lambda result: (
                    result.start,
                    result.end,
    ...
    def _keep_result(self, value: str, result) -> bool:
        """Réduit les faux positifs PERSON médicaux."""
        if result.entity_type != "PERSON":
            return True

        candidate = value[result.start:result.end]
        words = re.findall(
            r"[A-Za-zÀ-ÖØ-öø-ÿ'’-]+",
            candidate
        )
        left_context = value[
            max(0, result.start - 40):result.start
        ]

        identity_context = re.search(
            r"(?i)\b(?:patient|patiente|monsieur|madame|"
            r"mr|mrs|ms|dr|docteur|nom|name|called)\s*$",
            left_context
        )

        return len(words) >= 2 or bool(identity_context)
...
class PrivacyReviewStatus:
    ...
    def _check_automatic_pii(self) -> None:
        """Recense les échecs d’anonymisation déjà détectés."""
        self.automatic_pii_remaining = [
            record["id"]
            for record in self.records
            if "PII_REDACTION_FAILED"
